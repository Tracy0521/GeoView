    ...
    <div class="workspace">
      <aside class="model-panel">
        ...
        <label v-for="(model,index) in filteredModels" :key="model.id" class="model-row">
          ...
          <span class="model-info" title="点击修改模型" @click.prevent.stop="openEdit(model)"><strong>{{ model.name }}</strong><small>{{ model.score ? model.score + ' 分 · ' : '' }}{{ formatSize(model.size) }}</small></span>
