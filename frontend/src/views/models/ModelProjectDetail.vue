  ...
  <main v-if="project" class="detail-page">
    ...
    <el-dialog v-model="uploadVisible" title="增加已训练模型" width="720px" :close-on-click-modal="false">
      ...
      <div v-else class="remote-source">
        ...
        <div v-if="onlineRemoteServers.length" class="validation-card">
          <div><strong>补齐逐类别指标</strong><small>只处理缺少 class_metrics.json 的 best.pt，已有结果会跳过。</small></div>
          <el-select v-model="validationServerId" size="small" placeholder="选择服务器"><el-option v-for="server in onlineRemoteServers" :key="server.id" :label="server.name" :value="server.id" /></el-select>
          <el-select v-model="validationDatasetPath" size="small" filterable placeholder="验证数据集"><el-option label="各模型原 args.yaml 数据集（推荐）" value=""/><el-option v-for="dataset in validationDatasets" :key="dataset.path" :label="dataset.label" :value="dataset.path" /></el-select>
          <el-button size="small" type="primary" :loading="validationLaunching" :disabled="!validationServerId||validationStatus.running" @click="startClassMetricsGeneration">{{ validationStatus.running ? '验证运行中' : '生成缺失指标' }}</el-button>
          <div v-if="validationStatus.running||validationStatus.status.total" class="validation-progress"><span>{{ validationStatusLabel }} · {{ validationStatus.status.completed||0 }}/{{ validationStatus.status.total||'—' }}<template v-if="validationStatus.status.device"> · {{ validationStatus.status.device.toUpperCase() }}</template></span><el-progress :percentage="validationProgress" :status="validationStatus.interrupted||(validationStatus.status.failed&&validationStatus.status.failed.length)?'exception':(validationStatus.status.finished?'success':'')" /><span v-if="validationStatus.status.failed&&validationStatus.status.failed.length">失败 {{ validationStatus.status.failed.length }} 个</span></div>
          <pre v-if="validationError" class="validation-error">{{ validationError }}</pre>
        </div>
        <div v-loading="remoteLoading" class="remote-model-list">
          <label v-for="model in remoteModels" :key="`${model.server_id}:${model.remote_path}`" :class="['remote-model-row',{selected:selectedRemotePath===model.remote_path&&selectedRemoteServer===model.server_id,disabled:model.sync_status==='synced'}]">
            ...
            <span class="remote-main"><strong>{{ model.name }}</strong><small>{{ model.server_name }} · {{ formatSize(model.size) }}</small><small class="remote-path">{{ model.remote_path }}</small><small :class="['results-state',{missing:!model.has_results}]">{{ model.has_results ? `results.csv · ${formatSize(model.results_size)}` : '未找到 results.csv' }}</small><small :class="['category-state',{missing:!model.class_count}]">{{ model.class_count ? `${model.class_count} 个类别：${model.class_names.join('、')}` : '未找到类别配置' }}</small><small :class="['class-metrics-state',{missing:!model.has_class_metrics}]">{{ model.has_class_metrics ? `逐类别指标已就绪 · ${model.class_metrics_count} 类` : '缺少 class_metrics.json，需要执行一次验证' }}</small></span>
...
export default {
  ...
  computed: {
    ...
    smallSampleDatasets(){return this.projectModels.filter(item=>this.selected.includes(item.id)).map(item=>({id:item.id,name:item.name,color:this.modelColor(item.id),values:this.smallSampleRows(item)})).filter(item=>item.values.length)},
    classRankingColumns(){const classes=[];this.projectModels.forEach(model=>this.categoryRows(model).forEach(item=>{if(item.class&&!classes.some(current=>current.label===item.class))classes.push({key:`class::${item.class}`,label:item.class,type:'classMetric',instances:Number(item.instances)||0})}));return classes.sort((left,right)=>left.instances-right.instances||left.label.localeCompare(right.label))},
    rankingColumns(){return[...this.baseRankingColumns,...this.classRankingColumns,...this.trailingRankingColumns]},
    rankingRows(){return this.projectModels.map(model=>{const row={id:model.id,model,name:model.name,framework:model.framework||'—',precision:this.lastMetric(model,'precision'),recall:this.lastMetric(model,'recall'),map50:this.lastMetric(model,'map50'),map5095:this.lastMetric(model,'map5095'),score:model.score!==''&&Number.isFinite(Number(model.score))?Number(model.score):(model.score||null),epochs:this.modelEpochs(model),date:model.training_date||(model.created_at||'').slice(0,10)};const samples=this.categoryRows(model);const isRawBaseline=/raw.*yolo26/i.test(model.name);this.classRankingColumns.forEach(column=>{const item=samples.find(sample=>sample.class===column.label);if(item&&Number.isFinite(Number(item.value)))row[column.key]=Number(item.value);else if(isRawBaseline){const source=this.projectModels.flatMap(current=>this.categoryRows(current)).find(sample=>sample.class===column.label&&Number.isFinite(Number(sample.baseline)));row[column.key]=source?Number(source.baseline):null}else row[column.key]=null});return row})},
    ...
    onlineRemoteServers(){return this.remoteServers.filter(server=>server.status==='online')},
    validationDatasets(){const values=new Map();this.remoteModels.filter(model=>model.server_id===this.validationServerId&&model.dataset_path).forEach(model=>values.set(model.dataset_path,{path:model.dataset_path,label:`${model.dataset_path}（${model.class_count||0} 类）`}));return [...values.values()]},
    validationProgress(){const total=Number(this.validationStatus.status.total)||0;const completed=Number(this.validationStatus.status.completed)||0;return total?Math.min(100,Math.round(completed*100/total)):0},
  ...
  methods: {
    ...
    categoryRows(model){
      ...
      if(!metrics)return[]
      if(Array.isArray(metrics.small_sample)&&metrics.small_sample.length)return metrics.small_sample.map(item=>({...item,value:Number.isFinite(Number(item.strpn))?Number(item.strpn):null}))
      if(!Array.isArray(metrics.class_metrics))return[]
      return metrics.class_metrics.filter(item=>item&&item.class&&!['all','overall','total'].includes(String(item.class).trim().toLowerCase())).map(item=>({...item,value:Number.isFinite(Number(item.ap5095))?Number(item.ap5095):null}))
    },
    smallSampleRows(model){
      const metrics=model&&model.metrics
      if(!metrics)return[]
      if(Array.isArray(metrics.small_sample)&&metrics.small_sample.length)return metrics.small_sample
      if(!Array.isArray(metrics.class_metrics))return[]
      return metrics.class_metrics.filter(item=>item&&item.class&&Number(item.instances)<100&&Number.isFinite(Number(item.ap5095))).map(item=>({class:item.class,instances:Number(item.instances)||0,baseline:Number.isFinite(Number(item.ap50))?Number(item.ap50):Number(item.ap5095),baseline_label:'AP@50',strpn:Number(item.ap5095),result_label:'AP@50-95'}))
    },
...
.remote-main .category-state{overflow:hidden;color:#3f78b9;text-overflow:ellipsis;white-space:nowrap}.remote-main .category-state.missing{color:#9aa3af}
.remote-main .class-metrics-state{color:#278e63}.remote-main .class-metrics-state.missing{color:#d37c30}
.validation-card{display:grid;grid-template-columns:minmax(180px,1.2fr) 145px minmax(240px,1fr) auto;align-items:center;gap:9px;margin-bottom:13px;padding:11px;border:1px solid #dce8f5;border-radius:9px;background:#f7fbff}.validation-card>div:first-child strong,.validation-card>div:first-child small{display:block}.validation-card>div:first-child small{margin-top:3px;color:#7e8b9b;font-size:10px}.validation-progress{grid-column:1/-1;display:grid;grid-template-columns:auto minmax(180px,1fr) auto;align-items:center;gap:12px;color:#3d759f;font-size:11px}.validation-progress :deep(.el-progress){width:100%}.validation-error{grid-column:1/-1;max-height:130px;overflow:auto;margin:0;padding:9px;border-radius:6px;background:#fff0f1;color:#bd3f49;font:10px/1.5 Consolas,monospace;white-space:pre-wrap}@media(max-width:780px){.validation-card{grid-template-columns:1fr}.validation-progress,.validation-error{grid-column:auto}.validation-progress{grid-template-columns:1fr}}
...
.charts-panel .compare-tip{
  display:flex;align-items:flex-start;flex-wrap:wrap;gap:9px 10px;
  min-height:58px;padding:15px 18px;
  background:linear-gradient(135deg,#fff 0%,#fbfcff 100%);
}
.charts-panel .compare-tip strong{
  display:flex;align-items:center;align-self:stretch;flex:0 0 108px;min-width:108px;
  padding-right:17px;border-right:1px solid #e8ebf1;color:#1b2738;
  font-size:14px;line-height:1.35;white-space:nowrap;
}
.charts-panel .compare-tip span{
  display:inline-flex;align-items:center;gap:8px;max-width:min(100%,340px);min-height:30px;
  padding:0 11px;border:1px solid #e5e9ef;border-radius:16px;background:#f7f9fc;
  color:#5f6c80;font-size:12px;line-height:1.3;box-sizing:border-box;
  transition:border-color .2s,background .2s,transform .2s;
}
.charts-panel .compare-tip span:hover{border-color:#ced8e6;background:#fff;transform:translateY(-1px)}
.charts-panel .compare-tip span i{
  width:9px;height:9px;border-radius:50%;box-shadow:0 0 0 3px rgba(24,33,50,.05);flex:none;
}
.charts-panel .compare-tip small{align-self:center;padding:7px 2px}
@media(max-width:780px){
  .charts-panel .compare-tip{gap:8px;padding:13px}
  .charts-panel .compare-tip strong{
    flex:1 0 100%;min-height:28px;padding:0 0 9px;border-right:0;border-bottom:1px solid #e8ebf1;
  }
